/**
 * seed-blogs-admin.mjs
 * Gets the service role key from Supabase CLI and upserts 2 featured blog posts.
 * Run with: node scripts/seed-blogs-admin.mjs
 */
...
const projectRef = new URL(envUrl).hostname.split('.')[0];
...
try {
    ...
    serviceKey = keys.find((k) => k.role === 'service_role')?.api_key;
} catch (err) {
    console.error('Failed to get API keys from Supabase CLI:', err.message);
    process.exit(1);
}

if (!serviceKey) {
    console.error('Service role key not found. Make sure you are logged in: npx supabase login');
    process.exit(1);
}

console.log('Got service role key. Connecting...');
